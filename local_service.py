...
try:
    ...
    while True:
        try:
            content = requests.get(lamp_state_api_url)
            if content.status_code == 200 and str(content.text).strip() != PREVIOUS_STATE:
                PREVIOUS_STATE = str(content.text).strip()
                if str(content.text).strip() == "True":
